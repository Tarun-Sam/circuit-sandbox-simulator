...
with st.expander("Formulas"):
    ...
    st.latex(r"M_{dB} = 20 \log_{10}\left(\frac{1}{\sqrt{1 + (f/f_c)^2}}\right)")
...
with st.form("rc_low_pass_form"):
    col1, col2 = st.columns(2)
    with col1:
        r_str = st.text_input("Resistance R (Ohm)", key="r_filter")
    with col2:
...
if submitted:
    ...
    if errors:
        ...
        magnitude = 1.0 / np.sqrt(1.0 + (freq / fc) ** 2)
