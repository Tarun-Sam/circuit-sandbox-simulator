...
if submitted:
    ...
    if v_peak is not None:
        helpers.require_positive("Peak Voltage Vp", v_peak, errors)
    ...
    if errors:
        ...
        if np.isclose(impedance, 0.0, atol=REACTANCE_TOLERANCE):
            ...
            if np.isclose(reactance, 0.0, atol=REACTANCE_TOLERANCE):
                circuit_state = "resonance / voltage and current in phase"
            elif reactance > 0:
                circuit_state = "inductive / current lagging"
            else:
                circuit_state = "capacitive / current leading"
            ...
            with col2:
                ...
                st.metric("Power Factor", f"{power_factor:.4f} ({circuit_state})")
