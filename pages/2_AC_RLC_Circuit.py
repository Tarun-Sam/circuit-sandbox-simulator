...
if submitted:
    try:
        R=helpers.parse_engineering_notation(r_str); L=helpers.parse_engineering_notation(l_str)
        C=helpers.parse_engineering_notation(c_str); V_peak=helpers.parse_engineering_notation(v_peak_str)
        f=helpers.parse_engineering_notation(f_str); omega=2*np.pi*f; Xl=omega*L
        Xc=1/(omega*C); X_total=Xl-Xc; Z=np.sqrt(R**2 + X_total**2); I_peak=V_peak/Z
        phase_angle_rad=np.arctan2(X_total,R); PF=np.cos(phase_angle_rad)
        ...
        fig, ax=plt.subplots(); t=np.linspace(0,3/f,500); v=V_peak*np.sin(omega*t); i=I_peak*np.sin(omega*t - phase_angle_rad)
