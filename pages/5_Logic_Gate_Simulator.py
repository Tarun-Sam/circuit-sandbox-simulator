...
# --- Formulas Section ---
with st.expander("Boolean Algebra Expressions"):
    st.markdown(
        """
        This tool simulates the output of basic logic gates based on their Boolean algebra expressions.

        - **AND:** The output is 1 only if *all* inputs are 1.
          $Q = A \cdot B$
        - **OR:** The output is 1 if *any* input is 1.
          $Q = A + B$
        - **NAND:** The inverse of an AND gate.
          $Q = \\overline{A \\cdot B}$
        - **NOR:** The inverse of an OR gate.
          $Q = \\overline{A + B}$
        - **XOR:** The output is 1 if the inputs are *different*.
          $Q = A \oplus B$
        - **NOT:** The output is the inverse of the single input.
          $Q = \\overline{A}$
        - **BUF (Buffer):** The output is equal to the single input.
          $Q = A$
        """
    )
...
if simulate_button:
    gate = st.session_state.gate_key; a = st.session_state.a_key
    result = 0
    if gate in ['NOT', 'BUF']:
